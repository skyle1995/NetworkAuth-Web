...
export function useVariable() {
  ...
  async function onSearch() {
    loading.value = true;
    try {
      const { code, data, count } = await getVariables({
          ...
          btnClick: ({ dialog: { options, index } }) => {
            ...
            async function handleSave() {
              try {
                ...
                let res;
                if (curData.id) {
                  res = await updateVariable(curData);
                } else {
  ...
  async function handleDelete(row) {
    try {
      await ElMessageBox.confirm(
        `确认删除变量 <strong style="color:red">${row.alias}</strong> 吗？<br><span style="color:red;font-size:12px;">注意：此操作不可恢复！</span>`,
        "提示",
        {
          type: "warning",
          dangerouslyUseHTMLString: true
      ...
      const { code, msg } = await deleteVariable({ id: row.id });
      if (code === 0) {
